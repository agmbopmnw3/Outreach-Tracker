...
import { 
  ...
  AlertTriangle, XCircle, Bell, Briefcase, Loader2
} from 'lucide-react';
...
const TEAM_PRIORITY: Record<string, number> = {
  ...
  'R2 Nandyal': 7,
  'R3 Ananthapur': 8,
  'R3 Anantapur': 8, // Handle spelling variation
  'R4 Dharmavaram': 9,
  'R5 Kadapa': 10,
  'NW3': 99
};
...
const InternalCamera = ({ onCapture, onCancel, isCompact = false }: { onCapture: (files: File[], loc: string) => void, onCancel?: () => void, isCompact?: boolean }) => {
  ...
  if (locating) {
    return (
      <div className="p-4 bg-blue-50 border border-blue-200 rounded-xl flex flex-col items-center justify-center gap-2">
        <Loader2 className="animate-spin text-blue-600" size={24} />
        <span className="text-xs font-bold text-blue-700">Fetching Address...</span>
      </div>
    );
  }

  return (
    <div className={`border-2 border-dashed border-slate-300 rounded-xl bg-slate-50 flex flex-col items-center justify-center text-center transition-colors hover:bg-slate-100 ${isCompact ? 'p-4' : 'p-8'}`}>
      <div className={`bg-blue-100 text-blue-600 rounded-full flex items-center justify-center mb-2 ${isCompact ? 'w-10 h-10' : 'w-14 h-14'}`}>
        <Camera size={isCompact ? 20 : 28} />
      </div>
      <p className="text-xs font-bold text-slate-600 mb-1">{isCompact ? "Add Photos" : "Take Photo / Upload"}</p>
      <p className="text-[10px] text-slate-400 mb-3">Location Auto-Tagged</p>
      
      <button 
        type="button"
        onClick={() => fileInputRef.current?.click()}
        className="px-4 py-2 bg-blue-600 text-white text-xs font-bold rounded-lg shadow-sm hover:bg-blue-700 active:scale-95"
      >
        Select Files
      </button>
      ...
      {onCancel && (
        <button type="button" onClick={onCancel} className="mt-3 text-[10px] font-bold text-slate-400 underline">Cancel</button>
      )}
    </div>
  );
};
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden border border-blue-200">
        <div className="bg-blue-50 p-6 border-b border-blue-100 flex justify-between items-start">
          <div className="flex items-start gap-4">
            <div className="p-3 bg-blue-100 text-blue-600 rounded-full animate-bounce"><Bell size={24} fill="currentColor" /></div>
            <div><h3 className="text-xl font-bold text-blue-900">Follow-ups Due Today</h3><p className="text-sm text-blue-700 font-medium mt-1">You have {tasks.length} client{tasks.length > 1 ? 's' : ''} scheduled.</p></div>
          </div>
            ...
            return (
              <div key={i} className="p-4 hover:bg-slate-50 transition-colors flex justify-between items-center group">
                <div>
                  ...
                  {isAdmin && owner && (
                    <p className="text-[10px] font-bold text-indigo-600 bg-indigo-50 px-1.5 py-0.5 rounded w-fit mt-0.5 mb-1">
                      {owner.name} • {owner.team} • {owner.role}
                    </p>
                  )}
...
const EditActivityModal = ({ activity, currentUser, onClose, onUpdate, onImageClick }: any) => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from('staff_activity').update({
          ...
          customer_activity: formData.customer_activity,
  ...
  const displayGallery = [
     ...(activity.gallery || (activity.image_url ? [activity.image_url] : [])),
        ...
        <div className="p-6 overflow-y-auto space-y-5">
          <form onSubmit={handleSave} className="space-y-4">
            ...
            <div className="pt-2 pb-2 border-t border-b border-dashed border-slate-200">
               <label className="block text-xs font-bold uppercase text-slate-400 mb-2">Evidence & Location</label>
               <div className="grid grid-cols-4 gap-2 mb-3">
                  ...
                  {isOwner && !showCamera && (
                    <button type="button" onClick={() => setShowCamera(true)} className="flex flex-col items-center justify-center bg-blue-50 border border-blue-200 rounded-lg text-blue-600 hover:bg-blue-100 transition-colors h-16">
                        <Plus size={20} /><span className="text-[9px] font-bold">Add</span>
                    </button>
                  )}
               </div>
               
               {showCamera && (
                 <div className="animate-in slide-in-from-top-2 mb-3">
                    <InternalCamera onCapture={handleNewGPSCapture} onCancel={() => setShowCamera(false)} isCompact />
                 </div>
               )}
               
               {newLocation ? (
                 <div className="text-[10px] text-green-600 font-bold flex items-center gap-1"><MapPin size={10}/> New Location: {newLocation}</div>
               ) : activity.location && (
                 <div className="text-[10px] text-slate-400 font-bold flex items-center gap-1"><MapPin size={10}/> Recorded: {activity.location}</div>
               )}
            </div>

            <div>
              <label className="block text-xs font-bold uppercase text-slate-500 mb-1">Status (Outcome)</label>
              <select value={formData.status} onChange={e => setFormData({...formData, status: e.target.value})} disabled={!isOwner} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl font-bold text-slate-700 outline-none disabled:opacity-70">
                <option>Interested</option><option>In Progress</option><option>Converted</option><option value="Closed">Not Interested</option><option>Pending</option>
              </select>
            </div>

            {(formData.status === 'Interested' || formData.status === 'In Progress') && (<div><label className="block text-xs font-bold uppercase text-slate-500 mb-1">Next Follow-up</label><input type="date" value={formData.follow_up_date} onChange={e => setFormData({...formData, follow_up_date: e.target.value})} disabled={!isOwner} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl font-bold text-slate-700 outline-none disabled:opacity-70" /></div>)}
...
// --- UPDATED: DEFAULTER MODAL (Sorted & Ordered) ---
const DefaulterModal = ({ defaulters, role, onClose, teamMembers }: { defaulters: any[], role: string, onClose: () => void, teamMembers: any[] }) => {
  ...
  // Sort Defaulters by TEAM HIERARCHY, then ROLE HIERARCHY
  const sortedDefaulters = useMemo(() => {
    return [...defaulters].sort((a, b) => {
      const teamA = TEAM_PRIORITY[a.team] || 99;
      const teamB = TEAM_PRIORITY[b.team] || 99;
      if (teamA !== teamB) return teamA - teamB; // Sort by Team First

      const roleA = teamMembers?.find(m => m.id === a.user_id)?.role;
      const roleB = teamMembers?.find(m => m.id === b.user_id)?.role;
      const priorityA = ROLE_PRIORITY[roleA] || 99;
      const priorityB = ROLE_PRIORITY[roleB] || 99;
      return priorityA - priorityB; // Sort by Role Second
    });
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-red-200">
        <div className="bg-red-50 p-6 border-b border-red-100 flex justify-between items-start">
          <div className="flex items-start gap-4">
            ...
            <div><h3 className="text-xl font-bold text-red-900">{!isManagement ? 'Activity Missing' : 'Team Alert'}</h3><p className="text-sm text-red-700 font-medium mt-1">{!isManagement ? "You did not record any activity yesterday." : `There are ${defaulters.length} staff members with zero activity yesterday.`}</p></div>
          </div>
          <button onClick={onClose} className="text-red-300 hover:text-red-500 transition-colors"><X size={24} /></button>
        </div>
        <div className="p-0 max-h-[60vh] overflow-y-auto">
          {!isManagement ? (
            <div className="p-6 text-slate-600 text-sm">Please ensure you update your daily activities.</div>
          ) : (
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {sortedDefaulters.map((d, i) => {
                  const staffRole = teamMembers?.find(m => m.id === d.user_id)?.role || '-';
                  return (
                    <tr key={i} className="hover:bg-slate-50">
                      <td className="px-4 py-3 font-bold text-slate-700">{d.name}</td>
                      <td className="px-4 py-3 text-slate-500 text-xs font-bold">{d.team}</td>
                      <td className="px-4 py-3 text-slate-500 text-xs">{staffRole}</td>
                      <td className="px-4 py-3 text-blue-500 font-mono text-xs">{d.phone}</td>
                    </tr>
                  );
                })}
              </tbody>
...
// *** FIXED: Z-INDEX INCREASED TO 10002 TO STAY ON TOP OF EDIT MODAL ***
const ImageModal = ({ imageUrl, onClose }: { imageUrl: string | null, onClose: () => void }) => {
  if (!imageUrl) return null;
  return (
    <div className="fixed inset-0 z-[10002] bg-black/95 flex items-center justify-center p-4 backdrop-blur-md animate-in fade-in duration-200" onClick={onClose}>
      <button onClick={onClose} className="absolute top-6 right-6 p-3 bg-white/20 text-white rounded-full hover:bg-white/30 transition-colors"><X size={28} strokeWidth={3} /></button>
      <img src={imageUrl} alt="Full View" className="max-w-full max-h-[85vh] object-contain rounded-xl shadow-2xl" onClick={(e) => e.stopPropagation()} />
    </div>
  );
};
...
const ActivityCard = ({ task, currentUser, currentProfile, onDelete, onSelect, onImageClick, teamMembers }: any) => {
  ...
  // *** FIXED: Get Live Team/Role data from teamMembers list instead of task snapshot ***
  ...
  return (
    <div onClick={(e) => { if(!e.target.closest('button') && !e.target.closest('a')) onSelect(task); }} className="bg-white rounded-[20px] p-5 shadow-[0_2px_8px_rgba(0,0,0,0.04)] border border-slate-100 relative group transition-all hover:shadow-md hover:border-blue-300 cursor-pointer active:scale-[0.99]">
      ...
      {/* UPDATED: SMALL COMPACT IMAGE SCROLL ROW */}
        ...
        <div className="flex gap-2 mb-3 overflow-x-auto pb-1 scrollbar-hide">
          {galleryImages.map((url: string, index: number) => (
             <img 
               ...
               className="w-16 h-16 object-cover rounded-lg border border-slate-200 cursor-zoom-in flex-shrink-0" 
         ...
         <div className="h-8 bg-slate-50 rounded-lg mb-3 flex items-center justify-center text-[10px] text-slate-400">No photos</div>
      )}
      ...
      <div className="flex justify-between text-[11px] font-bold text-slate-400"><span className="flex items-center gap-1"><Calendar size={14}/> {task.follow_up_date || 'None'}</span><span className="flex items-center gap-1"><Clock size={14}/> {new Date(task.created_at).toLocaleDateString()}</span></div>
    </div>
  );
};
...
function ActivitiesView({ 
  tasks, initialFilter = 'ALL', onBack, onDelete, onSelect, onImageClick, user, profile, onPrint, teamMembers, 
  dateFilter, setDateFilter, 
  teamFilter, setTeamFilter, 
  roleFilter, setRoleFilter, 
  availableRoles, isGlobalAdmin
}: any) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-10">
      ...
      {/* UPDATED: ADDED ALL FILTERS TO ACTIVITIES VIEW */}
      <div className="p-4 flex gap-2 overflow-x-auto scrollbar-hide">
         {/* DATE FILTER */}
         <div className="relative min-w-[130px]">
            <input 
              type="date" 
              className="w-full appearance-none bg-white text-slate-600 text-xs font-bold py-2 pl-3 pr-3 rounded-lg outline-none border border-slate-200"
              value={dateFilter} 
            ...
            {dateFilter && (
              <button 
                ...
                className="absolute right-8 top-1/2 -translate-y-1/2 p-1 text-red-400 hover:text-red-600"
              >
                <X size={12} />
              </button>
            )}
...
function AdvancedAddActivity({ profile, onBack, onSave }: any) {
  ...
  const [form, setForm] = useState({ activityType: 'Customer Visit', customerType: 'New Customer', name: '', phone: '', customerActivity: '', notes: '', interestStatus: 'Interested', date: '' });
  ...
  const handleSubmit = async (e: any) => {
    ...
    let finalType = form.activityType === 'Customer Visit' ? `${form.customerType} Visit` : form.activityType;
    let finalStatus = 'Completed'; 
    if (['Customer Visit', 'Follow-up'].includes(form.activityType)) {
    ...
    try {
      ...
      const { error: insertError } = await supabase.from('staff_activity').insert([{
        ...
        follow_up_date: (form.interestStatus === 'Interested') ? form.date : null, role: profile?.role || 'Staff', assigned_by: profile?.name || 'Self', user_id: profile?.id, gallery: uploadedUrls, image_url: uploadedUrls[0] || null 
      }]);
  ...
  return (
    <div className="min-h-screen bg-white pb-20">
       ...
       <form onSubmit={handleSubmit} className="p-6 space-y-5 max-w-lg mx-auto">
          ...
          <div><label className="text-xs font-bold text-slate-400 uppercase mb-1 block">Activity Type</label><select name="activityType" value={form.activityType} onChange={handleChange} className="w-full p-4 bg-slate-50 rounded-xl font-bold border-none outline-none"><option>Customer Visit</option><option>Follow-up</option></select></div>
          ...
          {form.activityType === 'Customer Visit' && <div className="animate-in fade-in slide-in-from-top-2"><label className="text-xs font-bold text-slate-400 uppercase mb-1 block">Customer Type</label><select name="customerType" value={form.customerType} onChange={handleChange} className="w-full p-4 bg-slate-50 rounded-xl font-bold border-none outline-none"><option>New Customer</option><option>Existing Customer</option></select></div>}
          ...
          <div className="space-y-3 pt-2 border-t border-slate-100">
             <label className="block text-xs font-bold uppercase text-slate-500 tracking-wider">Photo Evidence & Location</label>
          ...
          {(form.activityType === 'Customer Visit' || form.activityType === 'Follow-up') && <div className="animate-in fade-in slide-in-from-top-2 pt-2 border-t border-dashed"><label className="text-xs font-bold text-slate-400 uppercase mb-1 block">Outcome</label><div className="grid grid-cols-1 gap-4"><select name="interestStatus" value={form.interestStatus} onChange={handleChange} className="w-full p-4 bg-slate-50 rounded-xl font-bold border-none outline-none"><option>Interested</option><option>Not Interested</option><option>Converted</option></select>{form.interestStatus === 'Interested' && <div className="animate-in fade-in slide-in-from-top-2"><label className="text-xs font-bold text-slate-400 uppercase mb-1 block">Follow Up Date</label><input type="date" name="date" value={form.date} onChange={handleChange} className="w-full p-4 bg-slate-50 rounded-xl font-bold outline-none text-slate-600" required /></div>}</div></div>}
